...
def build_list_length_dict2(
    word_list: List[str] = import_word_list(),
) -> Dict[str, List[int]]:
    c = color_string
    dict = {
        g: [len(filter_from_word_info(g, c(g, t), word_list)) for t in word_list]
        for g in word_list[:10]
    }
    return dict
...
def list_lengths_one(w, wl=import_word_list()):
    c = color_string
    return min([len(filter_from_word_info(w, c(w, t), wl)) for t in wl])
